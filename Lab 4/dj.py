...
# smoothing for cutoff updates (0..1], higher = snappier
LPF_SMOOTH_ALPHA = 0.25
...
try:
    ...
    while True:
        ...
        if now - last_update_time >= UPDATE_INTERVAL:
            ...
            smoothed_cutoff = (1.0 - LPF_SMOOTH_ALPHA) * \
                smoothed_cutoff + LPF_SMOOTH_ALPHA * new_target_cutoff

            with audio_lock:
                current_pitch = new_pitch
                # Update filter coefficients if samplerate available
                if current_samplerate > 0:
                    _recompute_filter_coeffs(
                        smoothed_cutoff, current_samplerate)
